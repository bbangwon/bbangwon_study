...
#region Public Methods
        ...
        public static void AddExplosion(Vector2 location, Vector2 momentum, int minPointCount, int maxPointCount, int minPieceCount, int maxPieceCount, float pieceSpeedScale, int duration, Color initialColor, Color finalColor)
        {
            ...
            for(int x=0;x<pieces;x++)
            {
                Effects.Add(new Particle(pieceLocation, Texture, ExplosionFrames[rand.Next(0, ExplosionFrames.Count)],
                    randomDirection(pieceSpeedScale) + momentum, Vector2.Zero, explosionMaxSpeed, duration, 
        ...
        {
            AddExplosion(location, momentum, 15, 20, 2, 4, 6.0f, 90, new Color(1.0f, 0.3f, 0f, 0.5f), new Color(0.0f, 0.0f, 0f, 0f));
        }
        ...
        public static void AddSparksEffect(Vector2 location, Vector2 impactVelocity)
        {
            ...
            for(int x=0;x<particleCount;x++)
            {
                Particle particle = new Particle(
                    ...
                    Texture, ParticleFrame, randomDirection((float)rand.Next(10, 20)),
