...
#endregion
#region Drawind
        static public void Draw(SpriteBatch spriteBetch)
        {
            ...
            for(int x = startX; x <= endX; x++)
                for(int y=startY; y<=endY;y++)
                {
                    ...
                    {
                        spriteBetch.Draw(texture, SquareScreenRectangle(x, y), tiles[GetTileAtSquare(x, y)], Color.White);
                    }
                }
        ...
        static public void GenerateRandomMap()
        {
            ...
            for (int x = 0; x< MapWidth;x++)
                for (int y = 0; y < MapHeight; y++)
                {
                    ...
                    if (rand.Next(0, 100) <= wallChanbcePerSquare)
                        mapSquares[x, y] = wallTile;
                }
